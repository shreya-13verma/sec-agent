"""Agent Workflow and Human-in-the-Loop Tests (TC-006, TC-007)."""
import pytest
from backend.app.agent.graph import compliance_agent

def test_tc_006_read_only_compliance_evaluation():
    # User queries compliance for a server
    initial_state = {
        "session_id": "test_sess_01",
        "user_query": "Which rules failed CIS benchmark audit for server 1001?",
        "intent": None,
        "thought_log": [],
        "tool_calls": [],
...
def test_tc_007_remediation_intent_triggers_interrupt_token():
    # User requests state-modifying action (patch/remediate)
    initial_state = {
        "session_id": "test_sess_02",
        "user_query": "Please patch and remediate pending security errata on server 1001",
        "intent": None,
        "thought_log": [],
        "tool_calls": [],
        "approval_required": False,
    ...
    assert result["remediation_plan"]["server_id"] == 1001
